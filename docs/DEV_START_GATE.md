...
- 🔴 **13 blockers (P0)** — must be answered before dev starts (§2).
...
| **B13** | **Visits & repeat treatment plans:** confirm the **visit types**, the **protocol follow-up intervals by disease state**, that a **treatment plan is one-to-many** (a new dated plan at each progression/toxicity), and that linking an entry to a visit stays **optional** | Reverses the earlier "no Encounter" decision; drives follow-up chasing and the overdue nudge | Clinical | As modelled in `Visits` + `Treatment_Plans` (workbook V3) |
