...
| **T4** | **Visit / encounter record** | ⚠️ **REVERSED** | ↩️ **Restored** | The clinical team has asked for **visits to be recorded** — the patient attends repeatedly, and follow-up must be chased. A `Visit` record is now in v1, with an **optional** link from clinical entries. Also makes `treatment_plan` **one-to-many**. See functional spec §1.6 / W12. |
